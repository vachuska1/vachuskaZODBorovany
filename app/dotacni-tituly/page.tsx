...
export default function GrantsPage() {
  ...
  const [documentUrl, setDocumentUrl] = useState("/api/pdfs/dotace")
  ...
  useEffect(() => {
    ...
    fetch("/api/documents/get-url?type=dotace")
      .then((res) => res.json())
      .then((data) => {
        setDocumentUrl(data.url || "/api/pdfs/dotace")
      })
      .catch(() => {})
  }, [])
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="w-full px-4 sm:px-6 lg:px-8">
        <h1 className="text-3xl font-bold text-center mb-8 text-gray-800">{t("grants")}</h1>

        <div className="w-full lg:w-[90%] mx-auto">
          <div className="bg-white rounded-lg shadow-lg overflow-hidden border border-gray-200">
            <div className="bg-gray-800 text-white p-4">
              <div className="flex justify-between items-center">
                <h2 className="text-xl font-bold">{t("grants")}</h2>
                <Button
                  onClick={handlePrint}
                  variant="outline"
                  size="sm"
